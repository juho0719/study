...
```
- 설치 후 webpack & webpack dev middleware 로딩
```javascript
...
```
- webpackDevMiddleware에 config 세팅 적용 및 번들링 파일 경로 지정
```javascript
var app = express();
var compiler = webpack(webpackConfig);

app.use(webpackDevMiddleware(compiler, {
    publicPath: webpackConfig.output.publicPath     // 일반적으로 output에 설정한 publicPath
    stats: { color: true }      // 번들링 시 webpack 로그 컬러 하이라이팅 
    // lazy: true,      // entry point에 네트워크 요청이 있을 때만 컴파일을 다시 함 
}));
app.listen(3000, function() {
    consoloe.log("Listening on port 3000!");
})
```
