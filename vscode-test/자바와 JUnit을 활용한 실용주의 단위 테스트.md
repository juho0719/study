...
- 루틴 두개가 공통된 코드를 호출한다면 모두 같은 결함을 공유하게 됨
- 검증을 위해 독립적인 방법을 찾아야 함
	- 곱셈 -> 제곱근 로직의 도치, DB에 데이터를 넣는 코드 -> JDBC쿼리 이용 등등
- 교차 검사는 모든 요소를 더하고 균형이 맞는지 확인하는 방법으로, 복식 부기에서 총 계정 원장을 맞추는 것과 같음

## Right-BI[C]EP: 다른 수단을 활용하여 교차 검사를 할 수 있는가?
- 문제에는 무수한 해법이 존재. 그 중 가장 최적화된 해법을 선택
- 그러면 프로덕션 결과를 교차 검사하기 위해 남은 덜 최적화된 해법이 남음
- 이처럼 너무 느리거나 유연하진 않지만 프로덕션 코드를 교차 검사하기 위한 방법으로 활용할 수 있음

## Right-BIC[E]P : 오류 조건을 강제로 일어나게 할 수 있는가?
- `Happy Path`가 있다는 것은 반대로 `Bad Path`도 있다는 것
- 오류가 발생하는 걸 방지하기 위해 테스트도 오류를 강제로 발생 시켜야 함
- 오류 혹은 다른 환경적인 제약 사항들을 확인하기 위해 고려해야 할 몇 가지 시나리오
  - 메모리가 가득 찰 때
	- 디스크 공간이 가득 찰 때
	- 벽시계 시간에 관한 문제들
	- 네트워크 가용성 및 오류들
...
- 어떤 코드가 특정 시간 안에 실행되는지 `assert`함
- 주의 사항
  - 성능을 측정할 때는 충분한 횟수를 반복해야 결과가 들쭉날쭉하지 않음
	- 반복하는 부분을 JVM이 최적화하지 못하는 지 확인해야 함
	- 최적화되지 않은 테스트는 매우 느림. 느린 테스트들은 빠른 것과 분리해야 함
	- 동일한 머신이라도 실행 시간은 다른 요소에 따라 달라질 수 있음
- 단위 성능 측정을 잘 사용하는 방법은 변경 사항을 만들 때 기준점(baseline)으로 활용하는 것
- 최적화하기 전에 먼저 기준점으로 경과 시간을 측정하고(몇 번 측정 후 평균으로 계산) 코드를 변경한 후 성능 테스트를 다시 실행, 결과 비교
- 성능이 핵심 고려 사항이면 단위 테스트보다는 고수준에서 문제를 집중하고 싶을 때는 `JMeter`같은 도구를 사용하면 됨
- 단위 수준 성능 측정에 관심이 많다면 `JUnitPerf`같은 서드파티 도구 활용
